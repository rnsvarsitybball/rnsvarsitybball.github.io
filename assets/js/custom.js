    ...
    if (width < 980){
        for (var i = 0; i<4; i++){
            ...
            var color = i+1;
            var new_html = current_html + '<article class="style'+color+' '+i+'game"><span class="image"><img src="'+game[i].game_pic+'" alt="" /></span><a href="'+game[i].game_link+'"><h4>#'+game[i].game_num+'</h4><h2>w/ '+game[i].game_team+'</h2><h3>'+game[i].game_date+'</h3><div class="content"><h4 class="score">'+game[i].game_score+'</h4><p>'+game[i].game_location+'</p></div></a></article>'
            $(".tiles").html(new_html);
        } 
    }
    else {
